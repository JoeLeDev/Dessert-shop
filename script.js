document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!emptyCartMessage)
    console.error('Element "empty-cart-message" is missing.');
    ...
    btnMinus.addEventListener("click", () => {
      ...
      if (productInCart) {
        ...
        if (quantityContainer.parentElement) {
          quantityContainer.parentElement.replaceChild(
            button,
            quantityContainer
          );
  ...
  function updateCartDisplay() {
    ...
    if (emptyCartMessage && orderTotal && confirmOrderButton && panier) {
      ...
      console.error(
        "Un ou plusieurs éléments requis sont manquants dans le DOM."
      );
    }
  }
});
